...
page_title: "xmft_st_audit_log_maintenance_application Resource - xmft"
subcategory: ""
description: |-
  
---

# xmft_st_audit_log_maintenance_application (Resource)
...
- `day_mode` (Number) default:0, The day mode. Set value 0 for months or 1 for days.
- `export_dir` (String) default:'', A full directory path specifying where the export files are. stored.
- `export_enabled` (Boolean) default:false, Define whether to export the deleted audit log entries to a file before they are deleted. If true then exportDir property is required to be set.
...
- `time_keeping_data_in_db` (Number) default:0, Specify in days or months (depending on 'dayMode' property) how old transfer log entries will be when they are deleted.
- `type` (String) default:AuditLogMaint, <nil>
